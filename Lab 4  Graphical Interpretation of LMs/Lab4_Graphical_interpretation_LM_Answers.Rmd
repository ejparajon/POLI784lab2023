...
<div align="left">$\large E[Body_mass_{i} | Flipper_length_{i}] = \beta_0 + \beta_1  Flipper_length_{i} + \epsilon,$</div>

where $E[Body_mass_{i} |  Flipper_length_{i}]$ denotes the expected body mass for penguin $i$ given the flipper length of penguin $i$. 
...
```
#Part 3: LM practice
Use the msleep dataset to model how body weight (bodywt) and brain weight (brainwt) influences total REM sleep (sleep_rem). Then create a model including an additional predictor (of your choice). Create a coefficient plot of the results of the two models and compare them.
