---
title: "Lab 11 - Missing Data and Imputation:Answers"
author: "Eric Parajon"
header-includes:
...
Below, use the function `na.omit()` to get rid of observations with missing values
```{r}
...
model_listwise <- lm(Temp~ Ozone + Solar.R + Wind, data = data_listwise)
...
data_recode$Ozone <- ifelse(is.na(data_recode$Ozone),
                            mean(data_recode$Ozone, na.rm = TRUE),
                            data_recode$Ozone)

data_recode$Solar.R<- ifelse(is.na(data_recode$Solar.R),
                            mean(data_recode$Solar.R, na.rm = TRUE),
                            data_recode$Solar.R)

data_recode$Wind<- ifelse(is.na(data_recode$Wind),
                            mean(data_recode$Wind, na.rm = TRUE),
                            data_recode$Wind)

data_recode$Temp <- ifelse(is.na(data_recode$Temp),
                            mean(data_recode$Temp, na.rm = TRUE),
                            data_recode$Temp)
...
#MICE TIME
mice_data <- mice::mice(data_miss, #call our data
...
#Using texreg package to display
...
#create running time variable, lets just assume we're in 2023
...
Try Amelia again, but this time let's use more of its functionality. Read in data from the `africa` package and examine its missingness. Then, using Amelia, impute missing values. Not that the dataset is cross-sectional by country, time series by year, and should have the `gdp_pc` variable specified as logged.
...
#use ts and cs so imputation algorithm also knows countries and years

africa_imp <- amelia(x = africa, cs = "country", ts = "year", logs = "gdp_pc")


models_africa <- list() 
for(i in 1:length(africa_imp$imputations)){
  models_africa[[i]] <- lm(gdp_pc ~ infl + trade + civlib + population,
                        data=africa_imp$imputations[[i]]) #why do we index like this?
}

summary(pool(models_africa))

#Putting into table

coefficient.names <-c("Intercept","Inflation","Trade","Civil Liberty","Population")
  
htmlreg(pool(models_africa),custom.coef.names = coefficient.names)


#alternatively!
all_coefs <- sapply(models_africa, coef) #get coefs
all_ses <- sapply(models_africa, function(x) sqrt(diag(vcov(x)))) #get ses

mi.meld(q = all_coefs, se = all_ses, byrow = F) 
```
