---
title: 'Lab 3: The Linear Model in R: Answer key'
author: "Eric Parajon"
output:
  html_document:
    df_print: paged
...
#Random error is always included in a regression model because unless there is a linear relationship between the variables in the model, there will always be some amount of error (or variation) which is not totally unaccounted for by the model.
```
...
#we just created three data frames - all with our x_vec_n values for "disp"
...
all(fit_n_1 == fit_n_1_a) #what does all() do here? #check in a vector whether all the values are true or not.
...
#Running model
bill_model <- lm(bill_length_mm ~ species+sex+body_mass_g, penguins)

summary(bill_model)

#Putting results in table (latex version)
stargazer(bill_model,title="Effect of Predictors on Bill Length",digits=3,star.cutoffs = c(0.1,0.05,0.01),
          label="bill_model",
          covariate.labels = c("\\textbf{Species} (reference= Adélie) &   \\\\
                      \\hspace{1cm}Chinstrap",
                      "\\hspace{1cm}Gentoo",
                    "\\textbf{Sex} (reference= Female) &   \\\\
                      \\hspace{1cm}Male",
                      "\\textbf{Body mass (grams)}"),
          notes="\\parbox[t]{12cm}{Coefficients reported from an OLS regression model. Significance codes:*$<$ 0.10,**p$<$ 0.05, ***p$<$ 0.01, two-tailed tests.}",
          notes.append=FALSE,
          style="apsr",
          dep.var.caption = c(""),
...
```{r, results='asis'}
#Putting results in table (html version)
stargazer(bill_model,title="Effect of Predictors on Bill Length",digits=3,star.cutoffs = c(0.1,0.05,0.01),
          label="bill_model",
          covariate.labels = c("\\textbf{Species} Chinstrap",
                      "\\textbf{Species} Gentoo",
                    "\\textbf{Male} (reference= Female)",
                      "\\textbf{Body mass (grams)}"),
          notes="\\parbox[t]{12cm}{Coefficients reported from an OLS regression model. Significance codes:*$<$ 0.10,**p$<$ 0.05, ***p$<$ 0.01, two-tailed tests. Species reference= Adélie}",
          notes.append=FALSE,
          style="apsr",
          dep.var.caption = c(""),
          dep.var.labels.include = FALSE,
