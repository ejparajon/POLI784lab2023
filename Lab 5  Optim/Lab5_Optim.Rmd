...
Some of today's code has been adapted from Simon Hoellerbauer, Isabel Laterzo and Sean Norton.
...
Let's suppose we're "hiking" up that mathematical hill trying to find the maximum, and using the first derivative to decide how big of a "step" we take from our current location. However, the first derivative only gives us information on the steepness of that hill. If all we're paying attention to is gaining elevation, we could take a very circular path up to the top. That's where the second derivative comes in.
...
Here we can see that across the 7 iterations, we get closer and closer to the root (which is 5.1). The algorithm continues (for as many iterations as you specify) until the parameter values stabilize, or $\theta_{n+1}$ and $\theta_{n}$ become approximately equal, and the root is approximated. If this doesn't make sense to you, watch \href{https://www.youtube.com/watch?v=-5e2cULI3H8}{this video}. 
...
y2 <- mtcars$______
X2 <-cbind(1,______,________)


#in practice, you can use other ways to select your starting values
#such as a grid search, but we will stick with selecting from a normal distribution
set.seed(2030)
params <- rnorm(4)


#now for the optim() part!
p2 <- optim(par = ____, #our starting parameters
           
           _____, #the function we want it to optimize
           
           ______, #return Hessian matrix (need for se)
           
           _______, #calls a quasi-Newton method
...
#the final element is the log of sigma2 
rse2 <- ____ #standard deviation of conditional distribution
____
