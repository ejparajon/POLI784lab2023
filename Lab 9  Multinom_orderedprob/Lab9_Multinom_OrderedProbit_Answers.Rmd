---
title: 'Lab 9: Multinomial and Ordered Probit Models: Answers'
author: "Eric Parajon"
output:
  html_document:
    df_print: paged
...
For ordered outcomes, we need to use the ordinal package (rather than base R). Otherwise, our syntax looks much the same.
...
#Now, our model:
m1 <- clm(apply ~ pared + public + gpa, 
           data = grad, link = "probit")
summary(m1) 
...
```{r}
tau1 <- m1$alpha[1] #extract latent thresholds (calculated w clm) from our model!
tau2 <- m1$alpha[2]
p1 <- invlogit(tau1) # 1
...
probs <- predict(m1, grad_2, type = "prob") #type = is different than for binomial model

#?predict.clm

head(rowSums(probs$fit)) #just to double check
cumul_probs <- t(apply(probs$fit, 1, cumsum)) #get the cumulative sum of the rows
...
coef_samp <- mvrnorm(1e3,coef(m1)[3:5]
                     ,vcov(m1)[3:5,3:5]) #keep relevant columns
...
taus <- m1$alpha #extract thresholds 
...
pacman::p_load(nnet,#for multinom()
               ...
               patchwork)#we will use it to put plots together (an extension to ggplot2)
...
#Combining plot using patchwork (could also use cowplot())
both<-mplot+fplot

#Adding annotations
both<-both + plot_annotation(tag_levels = 'a')

both<-both + plot_annotation(tag_levels = list(c("Male","Female")))& 
  theme(plot.tag.position = c(0, 1),
        plot.tag = element_text(hjust = -.5, vjust = -0.20))
...
#combining
both2<-cap_plot+nocap_plot

#Adding annotations
both2<-both2 + plot_annotation(tag_levels = 'a')

both2<-both2 + plot_annotation(tag_levels = list(c("Cap: Yes","Cap: No")))& 
  theme(plot.tag.position = c(0, 1),
        plot.tag = element_text(hjust = -.5, vjust = -0.20))
...
Generate a cumulative predicted probability plot varying the age of the respondent.
...
#Create the model
warm_model <- clm(warm~male+white+age+ed+prst, 
           data = hw, link = "probit")
summary(warm_model) 
...
#What is the effect of increasing each predictor by one unit?
#How do the odds of moving from a lower to adjacent higher category change?
coefs <- warm_model$coefficients[5:8]
#odds ratios
exp(coefs)
...
#Compute age quartiles and create hypothetical age values between min and max
age_quarts <- with(hw,c(min(age,na.rm = TRUE), max(age,na.rm=TRUE)))

age_rug <- with(hw,seq(age_quarts[1], age_quarts[2], length.out=100))

#Finding modal values
table(hw$male); table(hw$white); mean(hw$ed); mean(hw$prst)
...
probs <- predict(warm_model, hw_2, type = "prob") #type = is different than for binomial model
head(rowSums(probs$fit)) #just to double check
cumul_probs <- t(apply(probs$fit, 1, cumsum)) #get the cumulative sum of the rows
#transpose into columns for plotting and merging with values of gpa
head(cumul_probs)
plot_data <- data.frame(age_rug, cumul_probs)

#plot data
ggplot(data=plot_data, aes(x=age_rug))+
    geom_smooth(data=plot_data, aes(x=age_rug, y=Strongly.disagree), se=FALSE)+
    geom_smooth(data=plot_data, aes(x=age_rug, y=Disagree), se=FALSE)+
    geom_smooth(data=plot_data, aes(x=age_rug, y=Agree), se=FALSE)+
      geom_smooth(data=plot_data, aes(x=age_rug, y=Strongly.agree), se=FALSE)+
    ...
    geom_ribbon(data=plot_data, aes(ymin=Disagree, ymax=Agree,
         ...
         geom_ribbon(data=plot_data, aes(ymin=Agree, ymax=Strongly.agree,
                                    fill="Strongly.agree"))+
    labs(x="Age of respondent", y="Cumulative Predicted Probability")+
    scale_fill_discrete(name="Warm Relationship w/ Child")+
    theme_bw()
